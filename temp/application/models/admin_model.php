...
class Admin_model extends CI_Model{
	...
	//GET MENU SUB LINKS
	//++++++++++++++++++++++++++
	public function echo_menu($type1="", $id_1="", $menu)
	{
		$myMenu = explode('&', $menu);
 ...
 public function get_sidebar_content($content)
 {
					  ...
					  if($this->upload->do_upload())
					  {
						//$file = array('upload_data'
						  ...
						  //$this->s3_model->upload_s3('assets/images/' . $file);
 						  $cdn = $this->UploudToNMHS3(base_url('assets/images/' . $file), 'mynamibia-eu/assets/images', $file);

						  //populate array with values
						  $data = array(
							  'img_file'=> $cdn->filePath,
							  'type' => 'gal_img',
							  'bus_id' => $bus_id,
							  'gal_id' => $gallery_id
	...
	function get_testimonial_lang_option_selected($lang){
		...
		if($config->result()){
			...
			if($lang == 'english') { $eng = 'selected="selected"'; }
			if($lang == 'german') { $ger = 'selected="selected"'; }
 ...
 //            			<td style="width:34%">'.substr(strip_tags($row->body),0,80).'</td>
	...
	public function get_topic_option_select_list($id)
	{
		  ...
		  if($query->result()){
			...
			foreach($query->result() as $row){
				...
				if($id == $row->topic_id) { $sel = 'selected'; } else { $sel = ''; }
	...
	public function get_option_gallery($option) {
		...
		if($gallery->result()){
			...
			foreach($gallery->result() as $row){
				...
				if($option == $row->gal_id) { $selected="selected"; }  else { $selected = ''; }
		...
		if($config->result()){
		    ...
		    foreach($components as $comps){
				...
				if($comps == 'members'){
					...
					echo '<li><a href="'. site_url('/').'admin/members/"><i class="fa-icon-group icon-white"></i><span class="hidden-tablet"> Members</span></a></li>';	
	...
	public function slct_parent_page_list($pid, $page_id)
	{
		...
		foreach($query->result() as $row){
			...
			$selected = '';
			if($pid == $row->page_id) { $selected = 'selected="selected"'; }
	...
	public function get_page_features($page_id)
	{
		...
		if($query->result()){
			...
			if($row->page_features == '') { $pf = array(); } else { $pf = json_decode($row->page_features); }
			...
			if($sidebar_comp == "checked") {echo '<input name="p_sidebars" type="checkbox" value="sidebars" id="page_sidebars" '.$sidebars_checked.' > Sidebars&nbsp;&nbsp;&nbsp;';}
			
			if($people_comp == "checked") {echo '<input name="p_people" type="checkbox" value="people" id="page_people" '.$people_checked.' > People&nbsp;&nbsp;&nbsp;';}
	...
	public function get_all_product_category_types($check, $tid, $id)
	{
		  ...
		  if($query->result()){
			...
			foreach($query->result() as $row){
				...
				if($tid==$row->cat_type_id) { $selected = 'selected="selected"'; } else { $selected = ''; }
		  ...
		  } else { echo '<option value="0">No Category Types</option>'; }
