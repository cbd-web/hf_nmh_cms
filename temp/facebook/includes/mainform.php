        ...
        <script type=text/javascript>
            function showFB(f) {
                ...
                if (f==1) {
                    ...
                    <div class=\"Row\"><label class=\"Label\">' . $lang['Picture'] . ' ' . $lang['URL'] . ': ' . ( $adminOptions[ 'imgurCID' ] ? '<small>( <a class=imgUp onclick=\"$(\'input[id=lefile]\').click();\">' . $lang['Upload'] . '</a> <span id=imgUpResult></span> )</small>' : '' ) . '</label><br/><input name=\"Picture\" id=\"Picture\" type=\"text\" size=\"80\"> </div>";
                } else if (f==3  || f==4) {
                    s.innerHTML = "<div class=\"Row\"><label class=\"Label\">' . $lang['Image'] . ' ' . $lang['Description'] . ': </label><br/><textarea id=\"Text\" name=\"Message\" cols=\"58\" rows=\"8\"></textarea></div><div class=\"Row\"><div class=\"RowSm\"><label class=\"Label\">' . $lang['Image'] . ' ' . $lang['URL'] . ': ' . ( $adminOptions[ 'imgurCID' ] ? '<small>( <a class=imgUp onclick=\"$(\'input[id=lefile]\').click();\">' . $lang['Upload'] . '</a> <span id=imgUpResult></span> )</small>' : '' ) . '</label><br/><input name=\"URL\" id=\"Picture\" type=\"text\" size=\"80\" /><br><input type=checkbox name=proxy>' . $lang['Use'] . ' ' . $lang['Image'] . ' ' . $lang['Proxy'] . '?</div></div>";
                } else if (f==5) {
...
$i = 0;
$message .= "<br><img style='vertical-align: middle;' src='img/facebookpage.png' width='16px' title='Facebook Pages' alt='Facebook Pages' />&nbsp;<strong>" . $lang['Pages'] . "</strong> (%%p%%):";
...
$j = 0;
$message .= "<br><img style='vertical-align: middle;' src='img/facebookgroup.png' width='16px' title='Facebook Groups' alt='Facebook Groups' />&nbsp;<strong>" . $lang['Groups'] . "</strong> (%%g%%):";
...
if ( $adminOptions[ 'imgurCID' ] )
    $script .= '
        $("#lefile").change(function() {
            ...
            reader.onload = function(e) {
                ...
                $.ajax({
                    ...
                    error: function(){
                            $("#FBform").unblock();
                            $("#imgUpResult").html("<img width=\"11px\" src=\"img/error.png\">");
                           },
        ...
        function fdone(data) {
            $("#FBform").unblock();
            if (data.success) {
                $("#Picture").val(data.data.link);
                $("#imgUpResult").html("<img width=\"11px\" src=\"img/check.png\">");
            }
        }';
$script .= '
    });
