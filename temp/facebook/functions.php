...
function showHTML( $message, $heading = "", $title = "", $footer = "", $endExec = true )
{
    ...
    $footer .= " | FBMPGP v." . $adminOptions[ 'version' ];
    $langs = glob( "lang/*.php" );
    ...
    if ( $adminOptions[ 'theme' ] != 'fbmpgp' )
        ...
        $head = "";
    if ( $lang['DIR'] == 'RTL' )
        $head .= "<style>body { direction: rtl !important; } .list-inline { direction: ltr !important; } a.visit,.place-right,.app-bar .app-bar-menu>li {float: left !important; }</style>";
    if ( isset( $warn ) ) {
