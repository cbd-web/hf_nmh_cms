...
abstract class REST_Controller extends CI_Controller
{
	 ...
	 * @var array
	 */
	private $allowed_http_methods = array('get', 'delete', 'post', 'put', 'options', 'patch', 'head');
	...
	*/
	 ...
	 * @return boolean
	 */
	private function _perform_ldap_auth($username = '', $password = NULL)
	{
		if (empty($username))
		{
	 ...
	 * @return boolean
	 */
	private function _perform_library_auth($username = '', $password = NULL)
	{
		if (empty($username))
		{
	...
	private function _prepare_basic_auth()
	{
		...
		// FastCGI Workaround Roland 2017-05-09
		elseif ($this->input->server('REDIRECT_HTTP_AUTHORIZATION'))
		{
			if (strpos(strtolower($this->input->server('REDIRECT_HTTP_AUTHORIZATION')), 'basic') === 0)
			{
				list($username, $password) = explode(':', base64_decode(substr($this->input->server('REDIRECT_HTTP_AUTHORIZATION'), 6)));
			}
		}
	...
	private function _prepare_digest_auth()
	{
		...
		elseif ($this->input->server('REDIRECT_HTTP_AUTHORIZATION'))
		{
			$digest_string = $this->input->server('REDIRECT_HTTP_AUTHORIZATION');
		}
		else
		{
			$digest_string = "";
	 ...
	 */
	private function _check_blacklist_auth()
	{
	...
	private function _log_access_time()
	{
