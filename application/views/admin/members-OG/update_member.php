...
<!--                                         <div class="control-group">
        ...
        <!-- Accept or Decline a member who registered -- Declined Member will be deleted from the db -->
        <!-- Built for Simonis Storm Securities only (for now) 6568 -->
        <?php if($bus_id == 6568 && $type == 'Members') { ?>
          <div class="box span4">
            <div class="box-header">
              <h2><i class="icon-th"></i><span class="break"></span>Update <?php echo ucwords($member_type);?> Status</h2>
              <div class="box-icon">
                <a href="#" class="btn-close"><i class="icon-remove"></i></a>
              </div>
            </div>
            <div class="box-content">
              ...
              <?php if(isset($status)){
                ...
                if ($status != 'draft') {
                  echo '<div class="alert alert-success"><strong>Status: Member is activated</strong></div>';
                  echo '<button type="submit" data-status="draft" data-id="'.$id.'" data-type="0" class="btn btn-inverse btn status" id="btn_decline">Deactivate</button>';
                } else {
                  echo '<div class="alert alert-info"><strong>Status: Member account not yet activated</strong></div>';
                  echo '<button type="submit" data-status="live" data-id="'.$id.'" data-type="0" class="btn btn-inverse btn status" id="btn_status">Activate</button>';
                }

              }?>

              <button type="submit" data-status="delete" data-id="<?php echo $id; ?>" data-type="<?php echo $type_id; ?>"  class="btn btn-inverse btn status">Delete</button>

              <div id="result_msg5"></div>
            </div>
          </div>
        <?php } ?>
        <!-- // Accept or Decline a member who registered -- Declined Member will be deleted from the db -->
        ...
        <!--LOG SUBSCRIBER OUT IF THEY ARE LOGGED IN-->
        <?php echo $this->members_model->get_subscriber_login_status_do($id) ?>
    ...
    <div class="modal hide fade" id="modal-member-delete">
      <div class="modal-header">
        ...
        <h3>Delete the <?php echo ucwords($member_type);?></h3>
      </div>
      <div class="modal-body">
        <div class="alert">
          <button type="button" class="close" data-dismiss="alert">&times;</button>
          <strong>Please Note!</strong> Are you sure you want to delete the current <?php echo ucwords($member_type);?>? All <?php echo ucwords($member_type);?> details will be removed. This proces is not reversible.
        </div>
  ...
  //LOGOUT SUBSCRIBER 
  $('#btn_logout_subscriber').bind('click' ,function(e) {
      
    e.preventDefault();
    
      var frm = $('#logout-subscriber');

      $('#btn_logout_subscriber').html('<img src="<?php echo base_url('/').'admin_src/img/loading_white.gif';?>" /> Working...');
      $.ajax({
        type: 'post',
        data: frm.serialize(),
        url: '<?php echo site_url('/').'admin/logout_subscriber_do';?>' ,
        success: function (data) {
           $('#autosave').val('true');
           $('#result_msg4').html(data);
           $('#btn_logout_subscriber').html('Logout Subscriber');
        }
      });
  ...
  $('.status').bind('click' ,function(e) {
    ...
    //var frm = $('#type-update');
    $(this).html('<img src="<?php echo base_url('/').'admin_src/img/loading_white.gif';?>" /> Working...');
    $.ajax({
      type: 'post',
      // url: '<?php //echo site_url('/').'admin/update_member_status' ?>/'+status+'/'+member_id+'/'+type_id ,
      url: '<?php echo site_url('/').'admin/update_member_status' ?>' ,
      data: {status: status, member_id: member_id, type_id: type_id},
      dataType: "json",
      success: function (data) {
        $('#autosave').val('true');
        ...
        if(data.reload){
          ...
          var options = {'text': data.msg,'layout':'bottomLeft','type':'success'};
          noty(options);
          ...
          setTimeout(function(){ 
            ...
            window.location = '<?php echo site_url('/').'admin/subscribers' ?>';
      ...
      error: function (data) {
        ...
        var options = {'text': data.msg,'layout':'bottomLeft','type':'error'};
        noty(options);
