...
class Admin extends CI_Controller
{
	...
	function get_sidebar($id)
	{
		...
		if ($query->result()) {
			...
			if ($row['sidebar_type'] == 'gallery') {
				$gallery_selected = 'selected';
			}
			if ($row['sidebar_type'] == 'contact') {
				$contact_selected = 'selected';
			}
			if ($row['sidebar_type'] == 'downloads') {
				$downloads_selected = 'selected';
			}
			if ($row['sidebar_type'] == 'feature_image') {
...
//				$error = 'Your cell number is not valid. A 081/085/060 number is required!';
		...
		if ($val == TRUE) {
			...
			$this->session->set_userdata('openai_api_key', $openai_api_key);
...
//		$pass = '123';
	...
	//setlocale(LC_ALL, 'en_US.UTF8');
	function clean_slug_str($str, $type, $replace = array(), $delimiter = '-', $action = 'update')
	{
		if (!empty($replace)) {
			$str = str_replace((array) $replace, ' ', $str);
		}
	...
	function ai_collaboration()
	{
		...
		if ($type != '' && $body != '') {
			if ($type == 'post') {
				$response = $this->contactChatGPT('Generate a 7 metatags and a meta description for an article that has the following body: ' . $body . '. Format your response as follows: metaKeywords: a,b,c || metaDescription: 1,2,3');
				...
				$this->db->insert('ai_requests', $data);

				if (isset($response['error']['message'])) {
					$message = $response['error']['message'];

					$popover = '<div class="alert alert-error">
         			<button type="button" class="close" data-dismiss="alert">×</button>
            		' . $message . '</div>';
					...
					$data = explode('||', $response['choices'][0]['message']['content']);

					$data['basicmsg'] = 'Meta Data Successfully Genereated!';

					$popover = "<script>var options = {'text':'" . $data['basicmsg'] . "','layout':'bottomLeft','type':'success'};
				            noty(options);</script>";

					$metaKeywords = str_replace('metaKeywords: ', '', $data[0]);
					$metaDescription = str_replace('metaDescription: ', '', $data[1]);
			...
			$popover = '<div class="alert alert-error">
         			<button type="button" class="close" data-dismiss="alert">×</button>
            		Please type an article body and a type</div>';


			echo json_encode(
				array(
					'status' => false,
					'message' => 'Please type an article body and a type',
					'popover' => $popover
				)
			);
		}
	}
	function contactChatGPT($query)
	{
