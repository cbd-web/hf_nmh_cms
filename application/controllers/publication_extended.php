...
class Publication_extended extends CI_Controller {
	 ...
	 * Roland Ihms
	 */
	function publication_extended()
	{
		parent::__construct();
		//error_reporting(E_ALL ^ (E_NOTICE | E_WARNING));
		$this->load->model('publication_extended_model');
		//force_ssl();
	}
	...
	public function update_category_do()
	{
		if($this->session->userdata('admin_id')){
			...
			$this->publication_extended_model->update_category_do();
	...
	public function update_category($category_id)
	{
		if($this->session->userdata('admin_id')){
			...
			$category = $this->publication_extended_model->get_category($category_id);
			$this->load->view('admin/publications_extended/update_category', $category);
	...
	public function publications()
	{
		if($this->session->userdata('admin_id')){
			...
			$this->load->view('admin/publications_extended/publications');
	...
	public function add_publication()
	{
		if($this->session->userdata('admin_id')){
			...
			$this->load->view('admin/publications_extended/add_pub');
	...
	public function update_publication($pub_id)
	{
		if($this->session->userdata('admin_id')){
			...
			$pub = $this->admin_model->get_publication_extended($pub_id);
			$this->load->view('admin/publications_extended/update_pub', $pub);
...
//				$error = 'Post Content Required';
		...
		if($val == TRUE){
			...
			$this->db->insert('publications_extended', $insertdata);
			...
			echo '<div class="alert alert-success">
					...
					window.location = "'.site_url('/').'publication_extended/update_publication/'.$pubid.'/";
					</script>
					';
		}else{
...
//				$error = 'Your cell number is not valid. A 081/085/060 number is required!';
		...
		if($val == TRUE){
			...
			$this->db->update('publications_extended', $insertdata);
			//success redirect
...
//				$error = 'Your cell number is not valid. A 081/085/060 number is required!';
	...
	public function add_pub_doc()
	{
		if($this->session->userdata('admin_id')){
			...
			$this->publication_extended_model->add_pub_doc();
	...
	function delete_publication($pub_id){
		...
		if($this->session->userdata('admin_id')){
			...
			$this->db->delete('publications_extended');
			//LOG
			...
			echo '<script type="text/javascript">
				   window.location = "'.site_url('/').'publication_extended/publications/";
				  </script>';
	...
	public function categories($id=0)
	{
		if($this->session->userdata('admin_id')){
			...
			$data['id'] = $id;
			$this->load->view('admin/publications_extended/categories', $data);
	...
	public function add_category()
	{
		if($this->session->userdata('admin_id')){
			...
			$this->publication_extended_model->add_category();
	...
	public function add_category_pub()
	{
		if($this->session->userdata('admin_id')){
			...
			$this->publication_extended_model->add_category_pub();
	...
	{
		$this->publication_extended_model->load_doc($pub_id);
	}
	...
	public function delete_category_pub($id)
	{
		if($this->session->userdata('admin_id')){
			...
			$this->publication_extended_model->delete_category_pub($id);
	...
	{
		$this->publication_extended_model->get_categories_current($pub_id);
	...
	public function delete_category($id)
	{
		if($this->session->userdata('admin_id')){
			...
			$this->publication_extended_model->delete_category($id);
	...
	{
		$this->publication_extended_model->get_all_categories($id);
