...
class Csv extends CI_Controller {
	...
	public function upload_subscriber_csv() {
		...
		if($this->session->userdata('admin_id')){
			...
			if($file_path = $this->csv_model->upload_csv()){
				...
				if ($this->csvimport->get_array($file_path)) {
					...
					foreach ($csv_array as $row) {
						...
						$sub_data['sname'] = $row['sname'];
						$sub_data['email'] = $row['email'];
						$sub_data['phone'] = $row['phone'];
						$sub_data['type'] = $row['type'];
						$sub_data['bus_id'] = $bus_id;
						$sub_data['city'] = $row['city'];
						$sub_data['address'] = $row['address'];
						$sub_data['country'] = $row['country'];
						$sub_data['company'] = $row['company'];
						$sub_data['fax'] = $row['fax'];
						$sub_data['region'] = $row['region'];
						$sub_data['website'] = $row['website'];


						//CHECK IF TYPE EXISTS
						$typex = $this->db->where('bus_id', $bus_id);
						$typex = $this->db->where('type',trim($row['type']));
						$typex = $this->db->get('subscriber_type');

						if($typex->result()){

							$type_row = $typex->row();
							$type_id = $type_row->sub_type_id;
						...
						//END CHECK TYPE EXISTS


						//CHECK IF USER ALREADY EXISTS FOR BUS
						...
						if($exists->result()){
							...
							$subscriber_id = $row_exists->subscriber_id;

						//IF NOT EXISTS
						} else {

							$this->db->insert('subscribers', $sub_data);
							$subscriber_id = $this->db->insert_id();

						}


						//CHECK IF ENTRY IS IN SUBSCRIBERS_TYPE_INT TABLE
						$int = $this->db->where('bus_id', $bus_id);
						$int = $this->db->where('subscriber_id', $subscriber_id);
						$int = $this->db->where('type_id', $type_id);
						$int = $this->db->get('subscriber_type_int');

						if(!$int->result()){

							$in3['bus_id'] = $bus_id;
							$in3['type_id'] = $type_id;
							$in3['type_name'] = trim($row['type']);
							$in3['subscriber_id'] = $subscriber_id;

							$this->db->insert('subscriber_type_int', $in3);
			...
			$this->session->set_flashdata('msg', "Records Updated!");

			$java = 'reload_members()';

			echo "<script>
					  $.noty.closeAll()
					  var options = {'text':'Records Updated!','layout':'bottomLeft','type':'success'};
					  noty(options);
					  ".$java."
				  </script>";
